function back0(chatId, bot,){
    bot.sendMessage(chatId,"Теперь, раз ты определился, что хочешь разрабатывать \"то, что под капотом\", то выберем язык, котор" +
...
function back3(chatId, bot,){
    bot.sendMessage(chatId,"Теперь, когда ты уже выучил основы языка программирования, на котором собираетесь работать, то необходимо выучить что же такое базы данных и как они работают. Начнём изучение с MySQL.\n" +
...
function back4(chatId, bot,){
    bot.sendMessage(chatId,"Сейчас, когда ты уже знаком с базами данных, необходимо иметь представление о протоколах передачи данных, чтобы понимать как они работают.\n" +
        "\n" +
